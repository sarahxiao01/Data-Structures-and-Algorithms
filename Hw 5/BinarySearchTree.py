'''Make sure your DoublyLinkedList file is in the same directory as this one
because the LinkedQueue class uses it as a data member'''
...
class BinarySearchTree(LinkedBinaryTree):
    class _Item:
        ...
        def __init__(self, k, v = None):  ##BST can call all linkedbinarytree methods
            self._key = k
            self._value = v
    ...
    #------------------------------- nonpublic utilities -------------------------------
    def _subtree_search(self, node, k):  ##BST key operations
        """Return the node having key k, or last node searched."""
        if k == node._element._key:                                   # found match
            return node                                         
        elif k < node._element._key:                                  # search left subtree
            if node._left is not None:
    ...
    def before(self, node):#predecessor(self,node)
    ...
    def after(self, node):# successor(self,node)
    ...
    def get_node(self, k):
        """Return the node associated with key (raise Error if not found)."""
        if self.is_empty():
            raise Empty('Tree is empty')
        else:
            node = self._subtree_search(self._root, k)
            if k != node._element._key:
                raise Exception('Not found: ' + repr(k))
            return node

    # insert(self,k) and delete_key(self,K) for creating a BST based on only keys. 
    # Here we used v = None as default for creating items using self._Item(k,None).
    
    def insert(self, k):
        """Insert key k, that is, (k,None) into the Binary Search Tree"""
        if self.is_empty(): #always check if empty first if there is Tree
            self.add_root(self._Item(k))     # from BinaryTree (class Tree) #Create an _Item first
        else:
            node = self._subtree_search(self._root, k)
            if node._element._key < k:
                self.add_right(node, self._Item(k))        # inherited from BinaryTree (class Tree)
            else:                                          # there is no checking for equality???
                self.add_left(node, self._Item(k))         # inherited from BinaryTree (class Tree)

    def delete_key(self, k):
        """Remove the node within the Tree that contains key k (raise Error if not found)."""
        if not self.is_empty():
            node = self._subtree_search(self._root, k) 
            if k == node._element._key:
                self.delete(node)                        # reuse the delete node function #no need to return k?
                return                                   # successful deletion complete
        raise Exception('Not found: ' + repr(k))
    
    # __setitem__, __getitem__,__delitem__ methods will be used to create a sorted map/dictionary 
    # based on key,value pairs, that is, we used self._Item(k,v).
    ...
    def __getitem__(self, k):
        ...
        if self.is_empty():
          ...
          if k != node._element._key:
            raise KeyError('Key Error: ' + repr(k))
          return node._element._value

    def __setitem__(self, k, v):
        """Assign value v to key k, overwriting existing value if present."""
        if self.is_empty():
          self._add_root(self._Item(k,v))     # from LinkedBinaryTree
        else:
          node = self._subtree_search(self.root(), k) ##search passes in root
          if node._element._key == k:
            node._element._value = v                   # replace existing item's value
            
            return  ##why is return needed?? do we return node? #different from insert, only sets key, this sets item value
          else:
            item = self._Item(k,v)     ##item is self._Item(k, v)
            if node._element._key < k:
              self._add_right(node, item)        # inherited from LinkedBinaryTree
            else:
              self._add_left(node, item)         # inherited from LinkedBinaryTree
    ...
    def __delitem__(self, k):
        ...
        if not self.is_empty():
          node = self._subtree_search(self.root(), k)
          if k == node._element._key:
            self.delete(node)                           # rely on positional version
            return                                   # successful deletion complete
        raise KeyError('Key Error: ' + repr(k))
    # __setitem__, __getitem__,__delitem__ methods will be used to create a sorted map/dictionary based on Itemkey,value pairs.
    ...
    def __iter__(self):
        ...
        while node is not None:
            yield node._element._key   #pay attention to whether it returns key or node
            node = self.after(node)  #always self. node 
