        ...
        if order == "desc":
            for i in range(len(self)):
                ...
                for ind in range(i+1, len(self)):
                    if self._A[ind] > self.A[max_ind]:
                        max_ind = ind
                self.A_[max_ind], self.A_[i] = self.A_[i], self.A_[max_ind]
        elif order == "asc":
            for i in range(len(self)):
                min_ind = i
                for ind in range(i+1, len(self)):
                    if self._A[ind] < self.A[min_ind]:
                        min_ind = ind
                self.A_[min_ind], self.A_[i] = self.A_[i], self.A_[min_ind]
        else: 
